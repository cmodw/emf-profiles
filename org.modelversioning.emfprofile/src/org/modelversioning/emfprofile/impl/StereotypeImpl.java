...
public class StereotypeImpl extends EClassImpl implements Stereotype {
	...
	public boolean isApplicable(EObject eObject, Extension extension,
		...
		return isApplicable(eObject)
				&& getApplicableExtensions(eObject, appliedExtensions)
						.contains(extension)
				&& checkOCLConstraint(eObject, extension);
	}
	 ...
	 * @generated NOT
	 */
	private boolean checkOCLConstraint(EObject eObject, Extension extension) {
		if(extension.getConstraints().isEmpty()) {
			return true;
		}
		Constraint invariant = null;
		OCL ocl = OCL.newInstance();
		Helper helper = ocl.createOCLHelper();
		helper.setInstanceContext(eObject);
		try {
			invariant = helper.createInvariant(extension.getConstraints());
		...
		Query invariantQuery = ocl.createQuery(invariant);
		return invariantQuery.check(eObject);
	}
	...
	public EList<Operation> getOperations() {
		...
		for(EOperation eoperation : getEOperations()){
			if(eoperation instanceof Operation){
				operations.add((Operation)eoperation);
			}
		}
		return operations;
	}
