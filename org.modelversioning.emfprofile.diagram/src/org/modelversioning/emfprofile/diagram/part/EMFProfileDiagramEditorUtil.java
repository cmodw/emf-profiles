...
public class EMFProfileDiagramEditorUtil {
	...
	public static boolean openDiagram(Resource diagram) throws PartInitException {
		...
		page.openEditor(new URIEditorInput(diagram.getURI()), EMFProfileDiagramEditor.ID);
		return true;
	}
	...
	public static Resource openModel(Shell shell, String description, TransactionalEditingDomain editingDomain) {
		...
		try {
			...
			EMFProfileDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + uri, we); //$NON-NLS-1$
			MessageDialog.openError(shell, Messages.EMFProfileDiagramEditorUtil_OpenModelResourceErrorDialogTitle,
					NLS.bind(Messages.EMFProfileDiagramEditorUtil_OpenModelResourceErrorDialogMessage, fileName));
		}
		return resource;
	}
	...
	public static void runWizard(Shell shell, Wizard wizard, String settingsKey) {
		IDialogSettings pluginDialogSettings = EMFProfileDiagramEditorPlugin.getInstance().getDialogSettings();
	...
	public static Resource createDiagram(URI diagramURI, IProgressMonitor progressMonitor) {
		...
		progressMonitor.beginTask(Messages.EMFProfileDiagramEditorUtil_CreateDiagramProgressTask, 3);
		final Resource diagramResource = editingDomain.getResourceSet().createResource(diagramURI);
		final String diagramName = diagramURI.lastSegment();
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(editingDomain,
				Messages.EMFProfileDiagramEditorUtil_CreateDiagramCommandLabel, Collections.EMPTY_LIST) {
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
					throws ExecutionException {
				Profile model = createInitialModel();
				attachModelToResource(model, diagramResource);

				Diagram diagram = ViewService.createDiagram(model, ProfileEditPart.MODEL_ID,
				...
				try {
					...
					diagramResource.save(
							org.modelversioning.emfprofile.diagram.part.EMFProfileDiagramEditorUtil.getSaveOptions());
				} catch (IOException e) {

					EMFProfileDiagramEditorPlugin.getInstance().logError("Unable to store model and diagram resources", //$NON-NLS-1$
							e);
				}
