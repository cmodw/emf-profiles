...
public class StereotypeEditPart extends ShapeNodeEditPart {
	...
	protected void createDefaultEditPolicies() {
		installEditPolicy(EditPolicyRoles.CREATION_ROLE,
				new CreationEditPolicyWithCustomReparent(EMFProfileVisualIDRegistry.TYPED_INSTANCE));
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof StereotypeNameEditPart) {
			((StereotypeNameEditPart) childEditPart).setLabel(getPrimaryShape().getFigureStereotypeNameFigure());
			return true;
		}
		return false;
	}
	...
	public EditPart getPrimaryChildEditPart() {
		return getChildBySemanticHint(EMFProfileVisualIDRegistry.getType(StereotypeNameEditPart.VISUAL_ID));
	}
		...
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == EMFProfileElementTypes.EAttribute_3001) {
				return getChildBySemanticHint(
						EMFProfileVisualIDRegistry.getType(StereotypeTaggedValueCompEditPart.VISUAL_ID));
			}
			if (type == EMFProfileElementTypes.Operation_3007) {
				return getChildBySemanticHint(
						EMFProfileVisualIDRegistry.getType(StereotypeMethodCompEditPart.VISUAL_ID));
			}
		}
