	...
	*/
	public EditPart getTargetEditPart(Request request) {
		if (request instanceof CreateViewAndElementRequest) {
			...
			if (type == EMFProfileElementTypes.EClass_3002) {
				return this;
			}
			...
			if (RequestConstants.REQ_CONNECTION_END.equals(request.getType())) {
				for (Object type : ((CreateUnspecifiedTypeConnectionRequest) request).getElementTypes()) {
					if (type instanceof IElementType) {
						...
						if (elementType.equals(EMFProfileElementTypes.EClassESuperTypes_4002)
								|| elementType.equals(EMFProfileElementTypes.EReference_4003)
								|| elementType.equals(EMFProfileElementTypes.Extension_4005))
