...
public class EMFProfileBaseItemSemanticEditPolicy extends SemanticEditPolicy {
	...
	public Command getCommand(Request request) {
		if (request instanceof ReconnectRequest) {
			...
			if (view instanceof View) {
				Integer id = new Integer(EMFProfileVisualIDRegistry.getVisualID((View) view));
	...
	private IElementType getContextElementType(IEditCommandRequest request) {
		IElementType requestContextElementType = EMFProfileElementTypes.getElementType(getVisualID(request));
	...
	public static LinkConstraints getLinkConstraints() {
		LinkConstraints cached = EMFProfileDiagramEditorPlugin.getInstance().getLinkConstraints();
		if (cached == null) {
			EMFProfileDiagramEditorPlugin.getInstance().setLinkConstraints(cached = new LinkConstraints());
		}
		return cached;
	}
	...
	public static class LinkConstraints {
		 ...
		 * @generated
		 */
		public boolean canExistExtension_4005(Stereotype container, Extension linkInstance, Stereotype source,
				EClass target) {
			return true;
		}
		...
		public boolean canExistEClassESuperTypes_4002(EClass source, EClass target) {
			try {
				if (target == null) {
					...
					Object targetVal = EMFProfileOCLFactory.getExpression(1, EcorePackage.eINSTANCE.getEClass(), env)
				...
				EMFProfileDiagramEditorPlugin.getInstance().logError("Link constraint evaluation error", e); //$NON-NLS-1$
				return false;
			}
		}
		...
		public boolean canExistEReference_4003(EReference linkInstance, EClass source, EClassifier target) {
			try {
				if (target == null) {
					...
					Map<String, EClassifier> env = Collections.<String, EClassifier>singletonMap("oppositeEnd", //$NON-NLS-1$
							EcorePackage.eINSTANCE.getEClass());
					Object targetVal = EMFProfileOCLFactory
							.getExpression(2, EcorePackage.eINSTANCE.getEClassifier(), env)
							.evaluate(target, Collections.singletonMap("oppositeEnd", source)); //$NON-NLS-1$
					if (false == targetVal instanceof Boolean || !((Boolean) targetVal).booleanValue()) {
						return false;
					} // else fall-through
