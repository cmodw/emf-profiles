 ...
 * @generated
 */
public class EMFProfileValidationDecoratorProvider extends AbstractProvider implements IDecoratorProvider {
	...
	public boolean provides(IOperation operation) {
		...
		return view != null && ProfileEditPart.MODEL_ID.equals(EMFProfileVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					EMFProfileDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				EMFProfileDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
